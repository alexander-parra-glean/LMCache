...
class LMCacheConnectorV1Impl:
    ...
    def request_finished(
        ...
        if request.status == RequestStatus.FINISHED_ABORTED:
            # Scheduler-side adapter has no lmcache_engine — the
            # storage cleanup runs in the worker-side adapter instance.
            # Without this guard, vLLM's hybrid KV cache scheduler
            # crashes EngineCore the first time it aborts a request,
            # because request_finished_all_groups delegates here on
            # the scheduler side where lmcache_engine is None.
